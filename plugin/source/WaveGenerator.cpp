...
// FAST RENDER (AP) :::::
void WaveGenerator::renderNextBlock (juce::AudioSampleBuffer& outputBuffer, int numSamples) {
  ...
  if ( myMode == ANTIALIAS )
  {
    ...
    myBlepGenerator.setLimitingFreq(static_cast<float>(relativeFreq)); // up to the 2nd harmonic ..
  ...
  double panGain[2];
  double panL_dB = -3*pow((2.0 - 2.0*pan), 3.0);    // 0 => 0dB, .5 => -3 dB, 1 => -24db
  panGain[0] = juce::Decibels::decibelsToGain(panL_dB);
  double panR_dB = -3*pow(2.0*pan, 3.0);    // 0 => 0dB, .5 => -3 dB, 1 => -24db
  ...
  outputBuffer.addFromWithRamp(0, 0, wave.getRawDataPointer(), numSamples, static_cast<float>(gainLast[0]),
    static_cast<float>(volume*panGain[0]));
  outputBuffer.addFromWithRamp(1, 0,
    wave.getRawDataPointer(), numSamples,
...
inline void WaveGenerator::buildWave (int numSamples) {
  ...
  if (fabs(phaseAngleTarget - phaseAngleActual) > DELTA)
  {
    ...
    phaseShiftPerSample = (phaseAngleTarget - phaseAngleActual)/(2*numSamples); // 2 here is smoooothing ...
  ...
  for (int i=0; i < numSamples; i++)
  {
    ...
    if (hardSync && (fabs(masterDeltaBase - slaveDeltaBase) > DELTA))
    {
      ...
      if (currentMasterAngle < actualCurrentMasterDelta)
      {
        ...
        // ADD the blep ...
        {
          ...
          double angleAtRoll = fmod(currentAngle + percBeforeRoll*actualCurrentAngleDelta, 2*juce::MathConstants<double>::twoPi);
          double angleBeforeRoll = fmod(angleAtRoll - delta, 2*juce::MathConstants<double>::twoPi);
        ...
        currentAngle = 2*juce::MathConstants<double>::twoPi - percBeforeRoll*actualCurrentAngleDelta;
    ...
    if (myMode != NO_ANTIALIAS && masterBlepOccurred == false && myWaveType != sine)
    {
      ...
      if (actualCurrentAngleDeltaSkewed < 0) actualCurrentAngleDeltaSkewed += juce::MathConstants<double>::twoPi;
      ...
      if (myWaveType == square)
      {
        ...
        if (fmod(currentAngleSkewed, juce::MathConstants<double>::twoPi) < actualCurrentAngleDeltaSkewed)
        {
          double percAfterRoll = fmod(currentAngleSkewed, juce::MathConstants<double>::twoPi)/actualCurrentAngleDeltaSkewed; // LINEAR interpolation
      ...
      else if (myWaveType == sawRise || myWaveType == sawFall ) // SAW
      {
        ...
        if (fmod(currentAngleSkewed, 2*juce::MathConstants<double>::twoPi) > actualCurrentAngleDeltaSkewed
            && fmod(currentAngleSkewed, juce::MathConstants<double>::twoPi) < actualCurrentAngleDeltaSkewed
            )
        {
      ...
      else if (myWaveType == triangle)
      {
        ...
        if (fmod(currentAngleSkewed + juce::MathConstants<double>::twoPi/2, 2*juce::MathConstants<double>::twoPi) < actualCurrentAngleDeltaSkewed
            || fmod(currentAngleSkewed + 3*juce::MathConstants<double>::twoPi/2, 2*juce::MathConstants<double>::twoPi) < actualCurrentAngleDeltaSkewed
            )
        {
          ...
          if (fmod(currentAngleSkewed + 3*juce::MathConstants<double>::twoPi/2, 2*juce::MathConstants<double>::twoPi) < actualCurrentAngleDeltaSkewed)
          {
            aboveNonlinearity = fmod(currentAngleSkewed + 3*juce::MathConstants<double>::twoPi/2, 2*juce::MathConstants<double>::twoPi);
            percAfterRoll = aboveNonlinearity/actualCurrentAngleDeltaSkewed;
          }
          else // 3*double_Pi/2
          {
            aboveNonlinearity = fmod(currentAngleSkewed + juce::MathConstants<double>::twoPi/2, 2*juce::MathConstants<double>::twoPi);
            percAfterRoll = aboveNonlinearity/actualCurrentAngleDeltaSkewed;
          }
    ...
    *waveData = static_cast<float>(getValueAt(currentAngleSkewed));
    ...
    lastSample = static_cast<double>(*waveData); // NOTE* the most recent sample, for computation purposes
...
double WaveGenerator::skewAngle(double angle) {
  ...
  jassert(skewedAngle >= 0 && skewedAngle <= 2*juce::MathConstants<double>::twoPi);
...
void WaveGenerator::moveAngleForward(int numSamples) {
  ...
  currentAngle = fmod(currentAngle + numSamples*modAngleDelta, 2*juce::MathConstants<double>::twoPi); // ROLL
}
