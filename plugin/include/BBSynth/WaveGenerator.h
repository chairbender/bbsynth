...
class WaveGenerator
{
  ...
  juce::Array<float> history; // a running averaged wave, for rendering purposes
...
public:
  ...
  void setPitchHz(double newFreq) {
    ...
    float angleDelta = static_cast<float>(cyclesPerSample * 2.0 * juce::MathConstants<double>::twoPi);
    setMasterDelta(static_cast<double>(angleDelta));
  }
  ...
  void setPhaseTarget(double angleToGetTo) {
    ...
    jassert(angleToGetTo >= -juce::MathConstants<double>::twoPi && angleToGetTo <=
      juce::MathConstants<double>::twoPi);
